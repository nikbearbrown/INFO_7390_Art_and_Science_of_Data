...
with DAG(
    ...
    def process_new_links_task(ti):
        ...
        excluded_urls = [
            ...
            "https://media.geeksforgeeks.org/auth-dashboard-uploads/appstore-%281%29.png",
        ...
        for article_id, page_url, title in links:
            try:
                ...
                article_title, title_keywords, cleaned_content, text_chunks, image_urls = scrape_webpage(page_url, extract_images=True)

                # Filter and process only .png images, excluding specified URLs
                image_embeddings = []
                png_image_urls = [url for url in image_urls if url.lower().endswith('.png') and url not in excluded_urls]
                ...
                for idx, img_url in enumerate(png_image_urls):
                    try:
                        ...
                        if response.status_code == 200:
                            # Extract simplified keywords from title
                            simplified_keywords = [kw.lower() for kw in title_keywords]

                            # Check if any simplified keyword matches the image URL
                            if any(kw in img_url.lower() for kw in simplified_keywords):
                                ...
                                logging.info(f"Processed .png image: {img_url} with matching keyword.")
                            else:
                                logging.info(f"Image URL skipped as no simplified keywords matched: {img_url}")
                        else:
